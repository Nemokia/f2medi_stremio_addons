...
| حالت | برای چه کسی؟ | چطور اجرا می‌شود؟ | وابستگی‌ها |
|---|---|---|---|
| **EXE مستقل** | کاربر نهایی (ویندوز) | دابل‌کلیک `F2Media.exe` | هیچ — فقط Stremio |
| **Source + GUI** | توسعه‌دهنده (ویندوز) | `python gui.py` یا `F2Media.bat` | Python + WSL + پکیج‌ها |
| **سرور لینوکس** | توسعه‌دهنده / هاست | `python main.py` | Python + پکیج‌ها |
...
- [معرفی](#معرفی)
- [شروع سریع](#-شروع-سریع-فقط-کاربران-ویندوز)
...
- [نصب و راه‌اندازی](#نصب-و-راهاندازی)
  - [(اختیاری) ساخت EXE مستقل](#اختیاری-ساخت-exe-مستقل-بدون-نیاز-به-پایتون)
  - [اجرای سرور روی لینوکس](#اجرای-سرور-روی-لینوکس)
- [اتصال به Stremio](#اتصال-به-stremio)
- [استفاده روی تلویزیون و دستگاه‌های دیگر](#استفاده-روی-تلویزیون-و-دستگاه‌های-دیگر)
...
├── f2media_standalone.py       # نقطهٔ ورود standalone EXE: GUI + --backend
...
├── gui_backend.py              # سرور کنترل (FastAPI روی پورت 9090) — توسط gui.py فراخوانی می‌شود
├── F2Media.bat                 # لانچر ویندوز: اول EXE مستقل، بعد pythonw fallback
├── build.bat                   # اسکریپت ساخت EXE مستقل با PyInstaller (فقط یک‌بار)
...
| مورد | نسخه | لازم برای |
|---|---|---|
| Python | **3.10+** (تست‌شده روی 3.12) | توسعه / ساخت EXE |
| pip | همراه پایتون | توسعه / ساخت EXE |
| Git | برای clone مخزن | توسعه |
| WSL | برای اجرای GUI از ویندوز | فقط حالت source (اختیاری) |
| شبکه | دسترسی به `f2medi.top` و `v3-cinemeta.strem.io` | اجرا |

> 💡 **کاربر نهایی** فقط نیاز به `F2Media.exe` دارد — بدون نیاز به Python, pip, Git یا WSL.
...
### (اختیاری) ساخت EXE مستقل (بدون نیاز به پایتون)

اگر می‌خواهید کاربر نهایی بدون نصب پایتون و WSL فقط با دابل‌کلیک اجرا کند:

1. از **ویندوز** (نه WSL) فایل `build.bat` را اجرا کنید — پایتون باید روی ویندوز نصب باشد.
2. اسکریپت خودکار PyInstaller و وابستگی‌ها را نصب کرده و فایل `dist\F2Media.exe` را می‌سازد.
3. `F2Media.exe` را به کاربر نهایی بدهید — فقط با دابل‌کلیک اجرا می‌شود.

```
build.bat              ← فقط یک‌بار اجرا شود (توسط توسعه‌دهنده)
dist\F2Media.exe       ← خروجی standalone (~40-50MB)
F2Media.bat            ← لانچر خودکار (EXE را اولویت می‌دهد)
```

> **نحوهٔ کار:** فایل `F2Media.exe` شامل همه چیز است (پایتون + tkinter + FastAPI + تمام ماژول‌ها). وقتی کاربر روی Connect کلیک می‌کند، EXE خودش را با پارامتر `--backend` دوباره اجرا کرده و سرور addon را به‌صورت local اجرا می‌کند. **WSL, Python, یا هیچ پکیجی روی سیستم کاربر لازم نیست.**

### اجرای سرور روی لینوکس

علاوه بر حالت ویندوز، این پروژه روی هر سرور لینوکسی هم اجرا می‌شود — بدون GUI، فقط سرور API:
...
سرور روی `http://0.0.0.0:8081` بالا می‌آید. سپس در Stremio آدرس `http://<IP-سرور>:8081/manifest.json` را نصب کنید.

> 💡 **تفاوت با حالت ویندوز:** روی لینوکس GUI نیست و کنترل فقط از طریق API انجام می‌شود. برای اجرا به عنوان سرویس پس‌زمینه از `systemd` یا `screen`/`tmux` استفاده کنید.

نمونه اجرا با systemd:

```ini
# /etc/systemd/system/f2media.service
[Unit]
Description=F2Media Stremio Addon
After=network.target

[Service]
Type=simple
User=www-data
WorkingDirectory=/opt/fardabin_stremio_addons
ExecStart=/opt/fardabin_stremio_addons/venv/bin/python main.py
Restart=always
RestartSec=5
...
```bash
sudo systemctl enable --now f2media
```

### (اختیاری) Exposing روی شبکه

سرور روی `0.0.0.0:8081` بالا می‌آید — از دستگاه دیگری (اندروید/تلویزیون) آدرس `http://<IP-سرور>:8081/manifest.json` را در Stremio نصب کنید.

- حالت **EXE**: خودکار روی `0.0.0.0` اجرا می‌شود.
- حالت **source** (`python main.py`): پیش‌فرض localhost است؛ برای دسترسی شبکه:
...
## استفاده روی تلویزیون و دستگاه‌های دیگر

افزونهٔ F2Medi روی هر دستگاهی که Stremio داشته باشد (تلویزیون، موبایل، تبلت) کار می‌کند، به‌شرطی که بتواند به سرور addon روی لپ‌تاپ/کامپیوتر وصل شود.

### مراحل نصب روی تلویزیون

1. **لپ‌تاپ:** دکمهٔ **Connect** را بزنید. در لاگ پنل آدرسی شبیه این نمایش داده می‌شود:
   ```
   Install on other devices:
     http://192.168.1.105:8081/manifest.json
   ```

2. **تلویزیون:** Stremio را باز کنید → **Add-ons** → آیکون جستجو → آدرس بالا را وارد کنید → Install.

### نکات مهم

- لپ‌تاپ و تلویزیون **حتماً** باید روی یک شبکه WiFi باشند.
- آدرس `127.0.0.1` روی تلویزیون کار **نمی‌کند** — باید از IP واقعی لپ‌تاپ استفاده کنید.
- اگر IP لپ‌تاپ عوض شد (مثلاً ری‌استارت)، addon روی تلویزیون را با IP جدید **دوباره نصب** کنید.

### ثابت کردن IP لپ‌تاپ (توصیه شده)

برای اینکه مجبور نباشید هر بار addon را دوباره نصب کنید، به لپ‌تاپ یک IP ثابت بدهید:

1. تنظیمات روتر را باز کنید (معمولاً `192.168.1.1`).
2. بخش **DHCP Reservation** یا **Static Lease** را پیدا کنید.
3. MAC Address لپ‌تاپ را وارد کرده و IP دلخواه (مثلاً `192.168.1.105`) را ثابت کنید.
4. بعد از اعمال، یک‌بار لپ‌تاپ را ری‌استارت کنید.

> 💡 اگر روتر این گزینه را ندارد، می‌توانید IP ثابت را مستقیماً از تنظیمات شبکه ویندوز تنظیم کنید: Settings → Network & Internet → Properties → IP assignment → Manual.
